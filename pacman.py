...
class PacMan:
    ...
    def move(self):
        if self.direction:
            # Extraire la direction de déplacement à partir de l'attribut `self.direction`.
            dx, dy = self.direction
        
            # Calculer les nouvelles coordonnées X et Y en fonction de la direction
            new_x = self.x + dx
            new_y = self.y + dy

            # Vérifier si la nouvelle position entre en collision avec un mur
            if self.board[new_y][new_x] == 0:
                # Mettre à jour la position de Pac-Man si aucun mur n'est rencontré
                self.x = new_x
                self.y = new_y

                # Convertir les nouvelles coordonnées de la grille en position à l'écran
                self.screen_pos = grid_to_screen(grid_pos=[self.x, self.y], tile_size=[self.size_grid, self.size_grid])

                # Mettre à jour la position du rectangle de Pac-Man dans l'interface
