    ...
    def handle_keypress(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RIGHT:
                # Vérifiez si Pac-Man peut se déplacer à droite sans entrer en collision
                if not self.check_collision((1, 0)):
                    # Si le déplacement est possible, définissez la nouvelle direction de Pac-Man vers la droite
                    self.pacman.set_direction((1, 0))
            elif event.key == pygame.K_LEFT:
                # Vérifiez si Pac-Man peut se déplacer à gauche sans entrer en collision
                if not self.check_collision((-1, 0)):
                    # Si le déplacement est possible, définissez la nouvelle direction de Pac-Man vers la gauche
                    self.pacman.set_direction((-1, 0))
            elif event.key == pygame.K_UP:
                # Vérifiez si Pac-Man peut se déplacer vers le haut sans entrer en collision
                if not self.check_collision((0, -1)):
                    # Si le déplacement est possible, définissez la nouvelle direction de Pac-Man vers le haut
                    self.pacman.set_direction((0, -1))
            elif event.key == pygame.K_DOWN:
                # Vérifiez si Pac-Man peut se déplacer vers le bas sans entrer en collision
                if not self.check_collision((0, 1)):
                    # Si le déplacement est possible, définissez la nouvelle direction de Pac-Man vers le bas
                    self.pacman.set_direction((0, 1))
